...
# Define the field format for "DistanceTraveled" as a float (assuming it is a float)
DistanceTraveled_format = "<f"
field_definitions = [
    ("IsRaceOn", "i"),
    ("TimestampMS", "I"),
    ("EngineMaxRpm", "f"),
...
# Find the index of the "DistanceTraveled" field in the field_definitions list
DistanceTraveled_index = next((i for i, (name, fmt) in enumerate(field_definitions) if name == "DistanceTraveled"), None)
...
for line in lines:
    ...
    # Unpack the "DistanceTraveled" field based on its index in field_definitions
    field_name, field_format = field_definitions[DistanceTraveled_index]

    field_size = struct.calcsize(field_format)
    field_data = data[DistanceTraveled_index * field_size:(DistanceTraveled_index + 1) * field_size]
